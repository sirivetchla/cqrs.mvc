...
{
    public class DeleteCommandHandler : ICommandHandler<CancelCommand>
    {
        private readonly IRepository<HotelAggregate> _repository;

        public DeleteCommandHandler(IRepository<HotelAggregate> repository)
        {
            _repository = repository;
        }

        public void Execute(CancelCommand command)
        {
            if (command == null)
            {
